...
ReturnCode ALTAlgo::preProcessImpl()
{
    ...
    // @TODO Dist from landmark is the same as to landmark because for beginning we think that graph is not oriented
    return ReturnCode();
}

ReturnCode ALTAlgo::runDijkstraSssp (DijkstraSsspAlgo& dijkstra, int source, std::vector<double>& distances) {
    ReturnCode rc = dijkstra.setSource(source);
    rc = rc != ReturnCode::OK ? rc : dijkstra.compute();

    for (int vertex = 0; vertex < graph.V; ++vertex) {
        distances[vertex] = dijkstra.getDistance(vertex);
    }
...
ReturnCode ALTAlgo::initLandmarks()
{
    ...
    ReturnCode rc = runDijkstraSssp(dijkstra, currentLandmark, distFromLandmarks[0]);
    if (rc != ReturnCode::OK)
    {
        return rc;
    }
    ...
    for (int i = 1; i < landmarksCount; ++i)
    {
        ...
        for (int vertex = 0; vertex < graph.V; ++vertex)
        {
            distToLs[vertex] = std::min(distToLs[vertex], distFromLandmarks[i][vertex]);
        }
    }
    return rc;
}
