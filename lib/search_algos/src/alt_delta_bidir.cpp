...
{
    ReturnCode rc = AStarDeltaBiDirAlgo::preProcessImpl();
...
double ALTDeltaBiDirAlgo::heuristic(int vertex, int target)
{
    ...
    {
        double h_to = distToLandmarks[i][vertex] - distToLandmarks[i][target];
        double h_from =
            distFromLandmarks[i][target] - distFromLandmarks[i][vertex];
...
ReturnCode ALTDeltaBiDirAlgo::runDijkstraSssp(DijkstraSsspAlgo &dijkstra,
                                              int source,
...
ReturnCode ALTDeltaBiDirAlgo::initLandmarks()
{
    /**
     * dist - "distance"
     * Ls   - "landmarks"
